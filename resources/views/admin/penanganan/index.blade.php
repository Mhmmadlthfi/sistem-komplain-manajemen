...
@section('container')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h5 class="fw-bold py-3 mb-4">
            <span class="text-muted fw-light">Penanganan /</span> Data Penanganan
        </h5>
        ...
        <div class="card">
            <div class="card-header p-3 border-bottom d-flex justify-content-between align-items-center">
                <h5 class="my-0">Data Penanganan</h5>
                <button class="btn btn-success " data-bs-toggle="modal" data-bs-target="#exportModal"><i
                        class='bx bx-export mb-1'></i> Export</button>
            </div>

            <div class="card-body border-bottom p-2">
                <form action="{{ route('handling.index') }}" method="GET">
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        @forelse ($handlings as $handling)
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="badge {{ $handlingStatusClass[$handling->status] ?? 'bg-label-primary' }} me-0">
                                        {{ $handling->status }}
                                    </span>
                                </td>
                                @can('aftersales')
                                    <td>
                                        <a href="{{ route('handling.show', $handling->id) }}"
                                            class="btn btn-sm btn-icon btn-outline-info">
                                            <i class='tf-icons bx bx-file-find'></i></a>
                                        <form action="{{ route('handling.destroy', $handling->id) }}" method="post"
                                            class="d-inline">
                                            @method('delete')
                                            @csrf
                                            <button class="btn btn-sm btn-icon btn-outline-danger"
                                                onclick="return confirm('Apakah anda yakin ingin menghapus data ini? Jika anda menghapus data ini, maka data keluhan masuk akan berubah statusnya menjadi &quot;Diterima&quot;. anda dapat menghapusnya pada menu &quot;Keluhan Masuk&quot; jika ingin menghapusnya.')"
                                                @if (!$handling->can_delete) disabled @endif><i
    ...
    @include('admin.penanganan.modal.export')
@endsection
