...
class MobileApiController extends Controller
{
    ...
    public function index()
    {
        ...
        $handlings = Handling::where('user_id', $user->id)
            ->orderByDesc('id')
            ->with(['user', 'complaint', 'sale.customer', 'sale.saleDetail.product', 'sale.saleDetail.warranty'])
    ...
    public function update(Request $request, Handling $handling)
    {
        ...
        $validatedData = $request->only(['initial_condition', 'action', 'repair_result', 'repair_notes', 'status', 'handling_location']);
